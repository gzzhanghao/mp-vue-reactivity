...
describe('watch', () => {
  ...
  it('should force trigger on triggerRef when watching a shallow ref', async () => {
  ...
  it('should force trigger on triggerRef when watching multiple sources: shallow ref array', async () => {
  ...
  it('watchEffect should not recursively trigger itself', async () => {
    const spy = vi.fn();
    const price = ref(10);
  ...
  it('computed refs should not trigger watch if value has no change', async () => {
    const spy = vi.fn();
    const source = ref(0);
  ...
  it.each(options)('$name', async (option) => {
    const count = ref(0);
    const cb = vi.fn();
  ...
  it('OnCleanup also needs to be cleaned', async () => {
    const spy1 = vi.fn();
    const spy2 = vi.fn();
    const num = ref(0);
  ...
  it("effect should be removed from scope's effects after it is stopped", () => {
    const num = ref(0);
    let unwatch: () => void;
    ...
    expect((scope as any).effects.length).toBe(1);
    unwatch!();
    expect((scope as any).effects.length).toBe(0);
  });
